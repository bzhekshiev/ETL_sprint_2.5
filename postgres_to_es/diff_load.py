...
class PostgresSaver:
    ...
    def get_genres(self, obj_name: str) -> List[str]:
        """Получение списка записей жанров"""
        cur = self.pg_conn.cursor()
        cur.execute(f'''SELECT distinct id, name
                        FROM public.movies_genre; 
                        ''')
        obj = cur.fetchall()
        res = [{'id':r[0],'name':r[1]} for r in obj]
        if obj == []:
            None
        return res
    
    def get_persons(self, obj_name: str) -> List[str]:
        """Получение списка записей жанров"""
        cur = self.pg_conn.cursor()
        cur.execute(f'''SELECT distinct id, CONCAT(last_name, first_name) as name
...
if __name__ == '__main__':
    ...
    with psycopg2.connect(**dsl, cursor_factory=DictCursor) as pg_conn:
        ...
        if not loading:
            ...
            genres = p.get_genres('movies_genre')
            es.load_to_es(genres, INDEX_GENRE)
            pers = p.get_persons('movies_person')
            es.load_to_es(pers, INDEX_PERSON)
